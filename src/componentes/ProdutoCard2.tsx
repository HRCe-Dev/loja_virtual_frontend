...
const ProdutoCard2: React.FC<ProdutoCardProps> = ({ produto }) => {
  return (
    <Link
      ...
      className="  relative flex flex-row sm:flex-col w-full  bg-[#F2F2F2] p-2 rounded-xl flex flex-col gap-2 group border-orange-300 hover:border hover:shadow-xl transition-shadow duration-300 "
    >
      {produto.promocao && (
        <div
      ...
      <div className="relative flex-shrink-0 max-sm:w-1/2 md:w-full aspect-[4/3] md:aspect-[4/3] overflow-hidden">
        <Image
        ...
        <div className="flex h-12 justify-between ">
          {produto.promocao ? (
            <div className="flex flex-col">
              <span className="text-gray-400 text-sm max-sm:text-xs line-through mr-2">
                <Moeda>{produto.preco}</Moeda>
              </span>
              <span className="text-xl max-sm:text-lg  font-bold text-gray-900">
                <Moeda>
          ...
          <BtnAdicionarCarrinho2
            className="max-sm:absolute right-1 bottom-4"
