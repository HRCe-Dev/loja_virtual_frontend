...
export default function CookieBanner() {
  ...
  return (
    <>
      {visivel && (
        <div className="fixed bottom-10 inset-x-4 md:inset-x-10 z-40 bg-white border border-gray-200 rounded-lg shadow-md px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-sm text-gray-700 text-center md:text-left">
            Este site utiliza cookies para melhorar a sua experiência. Ao clicar em <strong>"Aceitar"</strong>, você concorda com a nossa&nbsp;
            <Link href="/politica-de-cookies" className="text-orange-500 hover:underline">
              Política de Cookies.
            </Link>
          </p>
      ...
      {mostrarModal && (
        <CookieModal onFechar={() => setMostrarModal(false)} />
      )}
    </>
  );
}
